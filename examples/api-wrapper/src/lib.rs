    ...
    let provided_pass = match req.headers().get(header::AUTHORIZATION) {
        ...
        Some(h) => match h.to_str() {
            ...
            Err(_) => {
                return Response::builder()
                    .status(StatusCode::INTERNAL_SERVER_ERROR)
                    .body(Body::from("cannot process auth header"))
            }
        },
    };
        ...
        Ok(s) => match s.as_str() {
            ...
            _ => {
                return Response::builder()
                    ...
                    .body(Body::from("Unsupported device status\n"))
            }
        },
    };
...
fn get_device_status(token: &str, device: &str) -> Result<String, StatusCode> {
    ...
    let json: Value = match from_str(
        String::from_utf8(rsp.body().to_vec())
            .or(Err(StatusCode::INTERNAL_SERVER_ERROR))?
            .as_str(),
    ) {
    ...
    let status = json
        ...
        .ok_or(StatusCode::INTERNAL_SERVER_ERROR)?
        .get(&"switch")
        .ok_or(StatusCode::INTERNAL_SERVER_ERROR)? // this is correct, "switch" two times, this is the structuire of this JSON schema
        .get(&"value")
        .ok_or(StatusCode::INTERNAL_SERVER_ERROR)?
        .to_string();
...
fn send_device_command(token: &str, device: &str, command: &str) -> Result<String, StatusCode> {
    let req = Request::builder()
        ...
        .body(Body::from(
            "{\"commands\": [{\"capability\": \"switch\", \"command\": \"".to_string()
                + command
                + "\"}]}",
        ))
        .or(Err(StatusCode::INTERNAL_SERVER_ERROR))?;
